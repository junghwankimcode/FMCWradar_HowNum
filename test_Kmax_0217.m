...
if isempty(gcp('nocreate'))
 ...
 detectionRate1(indexDoFlim) = sum(sum(temp_test_result1 .* temp_testAns)) / nTest
 detectionRate2(indexDoFlim) = sum(sum(temp_test_result2 .* temp_testAns)) / nTest
 detectionRate3(indexDoFlim) = sum(sum(temp_test_result3 .* temp_testAns)) / nTest
 detectionRate4(indexDoFlim) = sum(sum(temp_test_result4.*temp_testAns))/nTest
 detectionRate5(indexDoFlim) = sum(sum(temp_test_result5 .* temp_testAns)) / nTest % MNOMP baseline
 detectionRate6(indexDoFlim) = sum(sum(temp_test_result6 .* temp_testAns)) / nTest % MVALSE baseline
 detectionRate7(indexDoFlim) = sum(sum(temp_test_result7 .* temp_testAns)) / nTest % 
 
end % for indexSNR
%% Figure: Compare Detection Rates
figure(2)
plot(trainDoFlimset, detectionRate1, 'r-o', 'linewidth', 1)
hold on
grid on
plot(trainDoFlimset, detectionRate2, 'b-.^', 'linewidth', 1)
plot(trainDoFlimset, detectionRate3, 'g:d', 'linewidth', 1)
plot(trainDoFlimset, detectionRate5, 'm--s', 'linewidth', 1)
plot(trainDoFlimset, detectionRate6, '--<', 'linewidth', 1)
plot(trainDoFlimset, detectionRate7, '-.|')
plot(trainDoFlimset, detectionRate4, 'k--+', 'linewidth', 1)
xlabel("K_max")
