...
if isempty(gcp('nocreate'))
 ...
 R = (R_max*rand(1))*ones(1,trainDoF); %rand(1, trainDoF);
...
if isempty(gcp('nocreate'))
 ...
 if testDoF > 0
     R = (R_max*rand(1))*ones(1,testDoF); %rand(1,testDoF);
 ...
 detectionRate1(indexSNR) = sum(sum(temp_test_result1 .* temp_testAns)) / nTest
 detectionRate2(indexSNR) = sum(sum(temp_test_result2 .* temp_testAns)) / nTest
 detectionRate3(indexSNR) = sum(sum(temp_test_result3 .* temp_testAns)) / nTest
 detectionRate4(indexSNR) = sum(sum(temp_test_result4.*temp_testAns))/nTest
 detectionRate5(indexSNR) = sum(sum(temp_test_result5 .* temp_testAns)) / nTest % MNOMP baseline
 detectionRate6(indexSNR) = sum(sum(temp_test_result6 .* temp_testAns)) / nTest % MVALSE baseline
 detectionRate7(indexSNR) = sum(sum(temp_test_result7 .* temp_testAns)) / nTest % 
...
plot(trainSNRset, detectionRate5, 'm--s', 'linewidth', 1)
plot(trainSNRset, detectionRate6, '--<', 'linewidth', 1)
plot(trainSNRset, detectionRate7, '-.|')
...
legend("Proposed method", "Deep learning (with s)", "Deep learning (with s_F)", "MNOMP",'MVALSE' ,'Deep learning-based (CNN-Range-Doppler responses)','CA-CFAR','location', 'best');